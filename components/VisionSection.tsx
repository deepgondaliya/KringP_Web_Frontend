        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6 mb-12">
            {scenarios.map((scenario, index) => (
              <div
                key={index}
                className="group flex items-center gap-4 p-6 bg-card border border-border rounded-2xl hover:border-primary/30 transition-all duration-300 hover:translate-y-[-2px]"
              >
                <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0 group-hover:bg-primary/20 transition-colors">
                  <scenario.icon className="w-7 h-7 text-primary" />
                </div>
                <p className="text-left text-foreground">{scenario.text}</p>
              </div>
            ))}
          </div>

          <div className="relative p-8 rounded-3xl bg-card/50 backdrop-blur-sm border border-primary/30 shadow-[0_0_40px_-10px_hsl(var(--primary)/0.3)] overflow-hidden">
